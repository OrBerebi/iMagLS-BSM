...
def calc_gammatone_spectrum(p, fs, f_lim, C):
    # arrange dims
    ...
    N = x_l.shape[0]  # ir length
    c = x_l.shape[1]  # ir num. of channels
    n = C.shape[1]    # num of ERB freq bands
    # set outputs
...
def calc_color(p,p_hat, fs, f_lim, C):

    p_fc     = calc_gammatone_spectrum(p, fs, f_lim, C)
    p_hat_fc = calc_gammatone_spectrum(p_hat, fs, f_lim, C)

    #out = torch.abs(10*torch.log10(torch.abs(p_hat_fc) / torch.abs(p_fc)))
    out = torch.abs(p_hat_fc) / torch.abs(p_fc)
